...
async function search(q='ficción contemporánea'){
 ...
 try{const r=await fetch(`https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(q)}&printType=books&maxResults=40&langRestrict=es`);const data=await r.json();const items=(data.items||[]).filter(x=>x.volumeInfo?.title).slice(0,12);statusEl.textContent=`${items.length} libros`;
